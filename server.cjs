...
function parseSubscribersString(text) {
  if (!text) return 0;
...
app.all("/api/youtube-channel", async (req, res) => {
  ...
  try {
    ...
    if (!response.ok) {
          ...
          try {
            ...
            for (const item of contents) {
              if (item.channelRenderer) {
                ...
                const crAvatar = cr.thumbnail?.thumbnails?.[0]?.url?.startsWith("//") ? "https:" + cr.thumbnail.thumbnails[0].url : cr.thumbnail?.thumbnails?.[0]?.url || "";
                const crHandle = cr.subscriberCountText?.simpleText?.startsWith("@") ? cr.subscriberCountText.simpleText : `@${targetHandle}`;
                res.json({
                  success: true,
                  channel: {
                    title: crTitle,
      ...
      res.status(404).json({ error: `Canal "${query}" n\xE3o foi encontrado no YouTube. Verifique o @ digitado.` });
      return;
    }
    html = await response.text();
    const match = html.match(/var ytInitialData = ({.*?});<\/script>/) || html.match(/ytInitialData = ({.*?});/);
    if (!match) {
      res.status(500).json({ error: "N\xE3o foi poss\xEDvel extrair os dados estruturados do canal no YouTube." });
      return;
    }
    ...
    let avatar = header?.image?.decoratedAvatarViewModel?.avatar?.avatarViewModel?.image?.sources?.[0]?.url || metadata?.avatar?.thumbnails?.[0]?.url || "";
    ...
    for (const row of rows) {
      for (const part of row.metadataParts || []) {
        ...
        } else if (text.toLowerCase().includes("v\xEDdeo") || text.toLowerCase().includes("video")) {
          vidsText = text;
